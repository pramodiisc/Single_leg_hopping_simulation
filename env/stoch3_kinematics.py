...
class Serial2RKinematics():
    ...
    def inverseKinematics(self, ee_pos, branch=">"):
        ...
        '''
        assert np.size(
            ee_pos) == 2, "Invalid! In serial 2r Inverse Kinematics, input end effector pos size != 2"
        assert np.min(np.isnan(
            ee_pos)) != 1, "Invalid! In serial 2r Inverse Kinematics, input end effector pos has NaN element"
        ...
        if ((x ** 2 + z ** 2) > (l1 + l2) ** 2) or ((x ** 2 + z ** 2) < (l1 - l2) ** 2):
            # if ((x ** 2 + z ** 2) - (l1 + l2) ** 2 > 0) or ((x ** 2 + z ** 2) - (l1 - l2) ** 2 < 0):
            # if ((x ** 2 + z ** 2) - (l1 + l2) ** 2 > 0):
            #print (x ** 2 + z ** 2, (l1 + l2) ** 2)
            #print("Point is outside the workspace")
            #unit_x = x / np.linalg.norm(([x, z]))
            #unit_z = z / np.linalg.norm(([x, z]))
            #x = unit_x * (l1+l2)
            #z = unit_z * (l1+l2)
            #valid = False
            return valid, q
    ...
    def forwardKinematics(self, q):
        ...
        '''
        [l1, l2] = self.link_lengths
        x = self.base_pivot + l1*np.array([-math.sin(q[0]), -math.cos(q[0])]) + l2*np.array(
            [-math.sin(q[0] + q[1]), -math.cos(q[0] + q[1])])
...
class Serial3RKinematics():
    def __init__(self,
        ...
        self.link_lengths = link_lengths
        self.base_pivot = base_pivot
        self.serial_2R = Serial2RKinematics([base_pivot[1], base_pivot[2]], [
                                            link_lengths[1], link_lengths[2]])
    ...
    def inverseKinematics(self, leg_ID, ee_pos, branch=">"):
        ...
        valid1, [q[1], q[2]] = self.serial_2R.inverseKinematics(
            [x_prime, z_prime], branch)
...
if __name__ == '__main__':
    ...
    valid, angles = s.inverseKinematics([0, -0.6], branch='<')
    if valid:
        print("IK, ", angles)
    else:
        print("invalid")
    pos = s.forwardKinematics([0, -PI/2])
    print("FK", pos)
