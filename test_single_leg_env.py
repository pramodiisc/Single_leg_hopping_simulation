...
for i in range(190):
    ...
    for _ in range(4000):
        ...
        # action = np.array([896.989,1.0855,901.0025,1.009])# For previous training
        # action=np.array([900.5387397315353, 0.8672765335999428, 899.2592474631965, 0.04289948392691929]) #trained for 4700 kp
        # action=np.array([902.321096521594, 3.682804175523155, 900.1792983606956, 0.6640177814799676]) #trained for 6000 kp
        action=np.array([898.3681347292417, 3.905550632744719, 899.4493696767773, 0.38320494171132363,1000,3])
        r, dev, desired_pos, actual_pos, power, base_position,torque=env.step(action)
        tau.append(np.fabs(torque[1]))
        tau2.append(np.fabs(torque[2]))
        reward.append(r)
        d.append(dev)
        desired_position.append(desired_pos)
        actual_position.append(actual_pos)
        power_per_step.append(power)
        base_pos.append(base_position[0])
        env.render()
    f2=open("hip.txt","a")
    f2.write(str(tau))
    f2.close()
    ...
    f.close()
    hip_t=np.fabs(np.mean(tau))
    knee_t=np.fabs(np.mean(tau2))
    hip.append(hip_t)
    knee.append(knee_t)
